...
def main():
    ...
    hands = mp_hands.Hands(
        ...
        model_complexity= 1,# Set model complexity (0 for lite, 1 for full, 2 for heavy)
    ...
    while True:
        ...
        time_fps = cap.get(cv.CAP_PROP_FPS)
        frame_count = int(cap.get(cv.CAP_PROP_POS_FRAMES))
        elapsed_time = frame_count / time_fps
        # Process Key (ESC: end) #################################################
        key = cv.waitKey(10)

        if not ret or key == 27:
            output_csv = r"C:\Users\91999\Documents\RP\Model test\Model 1\Application\Top - MTM - Right\hand-gesture-recognition-mediapipe-main\output.csv"# ESC
            with open(output_csv , 'w', newline='') as csvfile:
                writer = csv.writer(csvfile)
                writer.writerow(['Output', 'Time (S)'])
                for output in output_array:
                    writer.writerow(output)
            print("Output has been written to 'output.csv'")

            # with open('output.csv', 'r') as input_file, open('modified_output.csv', 'w', newline='') as output_file:
            #     reader = csv.reader(input_file)
            #     writer = csv.writer(output_file)
            #
            #     for row in reader:
            #         # remove first and last element from row
            #         row = row[1:-1]
            ...
            grouped_csv = r"C:\Users\91999\Documents\RP\Model test\Model 1\Application\Top - MTM - Right\hand-gesture-recognition-mediapipe-main\grouped_file.csv"
            grouped_df.to_csv(grouped_csv, index=False)

            print("Same values grouped in 'grouped.csv'")


            heatmap_csv= r"C:\Users\91999\Documents\RP\Model test\Model 1\Application\Top - MTM - Right\hand-gesture-recognition-mediapipe-main\Heatmap.csv"
            with open(heatmap_csv, 'w' , newline='') as csvfile:
                writer = csv.writer(csvfile)
                writer.writerow(['x','y','Time'])
                for coordinates in wrist_tracker:
                    writer.writerow(coordinates)

            print("Heatmap file is Updated")
            # exec('speed_extractor')
            break
        ...
        if results.multi_hand_landmarks is not None:
            ...
            for hand_landmarks, handedness in zip(results.multi_hand_landmarks,results.multi_handedness):
                ...
                # Bounding box calculation
                    ...
                    landmark_list_with_time = landmark_list[0].copy()
                    landmark_list_with_time.append(elapsed_time)
                    # print(landmark_list_with_time)
                    wrist_tracker.append(landmark_list_with_time)
                    ...
                    output_array.append([keypoint_classifier_labels[hand_sign_id], elapsed_time])
                    # print(results.multi_hand_landmarks[0])
                    # print (pre_processed_point_history_li`st)
...
def calc_landmark_list(image, landmarks):
    ...
    for _, landmark in enumerate(landmarks.landmark):
        ...
        # if _ == 0:
        #     wrist_tracker.append([landmark_x, landmark_y])
...
def draw_info_text(image, brect, handedness, hand_sign_text,
    ...
    if hand_sign_text != "":
        ...
        # translation_table = str.maketrans("abcdefghijklmnoPqrstuvwxyz", "ɒdɔbɘʇϱʜiįʞlmnoqpɿƨɈυvwxγz")
        # info_text = info_text.translate(translation_table)
        cv.putText(image, info_text, (brect[0] + 5, brect[1] - 4),
               cv.FONT_HERSHEY_SIMPLEX, 0.6, (55, 255, 255), 1, cv.LINE_AA)
